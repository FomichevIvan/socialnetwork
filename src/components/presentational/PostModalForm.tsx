    ...
    return (
        ...
        <Box sx={style}>
            ...
            <div className={'row-cont'}>
                <Input multiline rows={6}  value={(title === null || title === undefined) ? '' : title } id={'title'} placeholder={'title'} onChange={onChange} className={'post-title-input'}/>
                <Input multiline rows={6} value={(body === null || body === undefined) ? '' : body } id={'body'} placeholder={'text' +
                    ' something'} onChange={onChange} className={'post-body-input'}/>
            </div>
            <div className={'row-cont'}>
                <Button disabled={!title && !body} onClick={onSubmit}>{id ? 'SAVE' +
